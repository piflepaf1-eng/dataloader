...
---------------------
Ce module fait exactement une chose : consommer un itérateur produit par un
``BackendProtocol`` et émettre des ``Batch``.  Il ne contient aucune logique
d'augmentation, de filtrage ou de composition de pipeline.
...
class _DALINode(BaseNode):  # type: ignore[misc]
    ...
    d'augmentation.  Il encapsule :

    - Le défilement de l'itérateur produit par le backend.
